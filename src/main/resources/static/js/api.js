...
// Models for the "Generate with AI" picker -- {id, name} pairs, either
// proxied from the aggregator's own /models endpoint or the server's
// configured static fallback list. Never fails the request itself; an
// unreachable/misconfigured aggregator just yields an empty/fallback list.
function listAiModels() {
  return request("/ai/models");
}

// `data` is {prompt, model, folderId?} -- creates and returns a full
// EntryResponse (title/summary/contentMarkdown/suggestedTags generated by
// the model), same shape createEntry() returns. Can take several seconds
// (a live LLM call) and can reject with a 502-derived message if the
// aggregator is unreachable or returns something unparseable.
function generateEntry(worldId, data) {
  return request(`/worlds/${worldId}/entries/generate`, {
    method: "POST",
    body: JSON.stringify(data),
  });
}
